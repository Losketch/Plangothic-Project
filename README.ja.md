...
<h1>プランゴシック・プロジェクト（Plangothic Project）</h1>
...
フォントファイルはSIL Open Font License 1.1でリリースされています。
...
#### あなたはできません：
- フォントファイル自体を単独で販売する（フォントはソフトウェア/機器の一部としてバンドル販売することは可能ですが、単独の商品としては販売できません）。
- フォントファイルの任意の部分を非SIL Open Font License発表します。
- 上記に記載されていないその他のSIL Open Font License違反行為。
 
**免責事項（非法律条項）：**
ユーザーは現地の法律と規制を遵守しながら自由にフォントを使用できます。作者はこのフォントの使用によるシステム損害または法的紛争について責任を負いません。OFLライセンス自体に免責条項が含まれています。詳細は [LICENSE-OFL.txt](LICENSE-OFL.txt) をご覧ください。

### ビルドと開発コード（MIT License）
- このリポジトリのソースコード、スクリプト、設定ファイルはMIT Licenseでリリースされており、商用目的を含む自由な使用、修正、配布が可能です。

---

**重要事項：**
- 本リポジトリの任意のコンテンツを使用することにより、該当するライセンスに従うことに同意したものとみなされます。詳細な条項については、[LICENSE-OFL.txt](LICENSE-OFL.txt)および[LICENSE-MIT.txt](LICENSE-MIT.txt)をご参照ください。
- 本フォントは無料で商用利用可能であり、OFLライセンスの下で公開されています。誰でも自由に使用、修正、配布することができます。作者はいかなる個人または組織に対しても、**本フォントファイルを単独で販売する**ことを許可したことはありません。フォントファイルを入手する際に料金の支払いを求められた場合、当該行為には誤解を招く、または詐欺のリスクが存在する可能性がありますので、十分にご注意ください。なお、OFLライセンスでは、フォントを他の有料ソフトウェアまたはハードウェアの一部としてバンドル配布することが許可されています。
- 作者がライセンス契約で享受する正当な権利を行使しない、適時に行使しない、または十分に行使しない場合でも、当該権利を放棄したとはみなされず、また作者が将来当該権利を行使することに影響を与えません。
- 本リポジトリのすべてのコンテンツには、いかなる政治的意図、隠喩、目的も含まれておらず、あらゆる政治活動とは無関係です。作者は、本フォントを使用することにより生じるいかなる連帯法的責任も負いません。
...
- 当プロジェクトは、[Unifont](https://unifoundry.com/unifont)と同じく、字形を表示するための補助フォントとして開発されており、組版への使用は想定していません。CTL を用いてこれらの文字を組版すことは、このプロジェクトの対象外である、同様に、異体字セレクタ（**I**deographs **V**ariation **S**elector, IVS）も本プロジェクトの対象外です。ご関心のある方は、各自で派生フォントを作成することができます。
...
### 6. 各配布版の違いは何ですか？
- **Super版**：すべての形式のフォントファイルを含むため、パッケージサイズが比較的大きいです。
- **Static版**：P1とP2の2つの独立したTTFフォントファイルと1つのTTCファイルを含みます。
- **OTF版**：OpenType形式のフォントファイルとTTC/OTCファイルを含みます。
- **Web版**：WOFF2形式のウェブフォントファイルです。
- **個別のTTFファイル**：Static版から直接抽出したもので、個別にダウンロードして使いやすくなっています。

**圧縮形式**：.7zと.zipは内容は同じで、圧縮方式が異なるだけです。.7zはより小さいアーカイブになりますが、サードパーティの解凍ソフトが必要な場合があります。.zipはやや大きめですが、ほとんどのOSで標準サポートされています。

<details>
<summary><b>付録：PlangothicフォントをWindowsプラットフォームにインストールし、グローバルフォールバックとして設定する方法：</b></summary>

**基本インストール方法：**

1. **フォントのインストール**：TTF版フォントファイル（2つの個別ttfファイルまたは単一のttcファイルのいずれかを選択）をダウンロードし、右クリックで「すべてのユーザー用にインストール」を選択（管理者権限が必要）；
2. **フォールバックの設定**：`registry/Plangothic-Install.reg` をダブルクリックして実行（管理者権限が必要）
3. **再ログイン**：設定を有効にするため、Windowsアカウントからログアウトして再ログイン
4. **アンインストール**：フォントファイルを削除するか、`registry/Plangothic-Uninstall.reg` を実行してレジストリ設定を削除

> **高度な使用方法と詳細説明**：[Experimental_README.md](./registry/Experimental/README.md) をご覧ください
</details>
...
5. 本プロジェクトは、主に以下のツールまたはオープンソースフォントプロジェクトの一部またはすべてを使用、ないし参考にしています：
    ...
    - [Unihanデータベース](https://www.unicode.org/Public/17.0.0/ucd/Unihan.zip)。
...
4. ***Planseal Project（プランシール・プロジェクト、中国語：遍篆體項目、遍篆体项目）***：『説文解字』小篆ベースに、CJK統合漢字拡張ブロックを補完するフォントの作成を目指すプロジェクトです、将来的にはプランゴシック・プロジェクトと統合され、その一部になる可能性があります。
    - **プロジェクトステータス**：Unicode 18.0は小篆を新たに追加したため、プランゴシック・プロジェクト、プランライト・プロジェクト、プランシェフト・プロジェクトと合併し、その一部となる。
