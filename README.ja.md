...
**利用できること：**
- 商用・非商用目的での無料使用（許可不要）
- ソフトウェア、ウェブページ、デバイスへの組み込み配布
- 修正およびフォーマット変換（修正版はOFLライセンスを維持し、「Plangothic」「遍黑」の名称使用禁止）
...
**重要事項：**
- 本リポジトリの内容を使用することで、それぞれのライセンスに従うことに同意したものとみなされます
- このフォントは完全無料です - 有料での入手は詐欺行為です
- 詳細な規定については[LICENSE-OFL.txt](LICENSE-OFL.txt)および[LICENSE-MIT.txt](LICENSE-MIT.txt)をご参照ください
