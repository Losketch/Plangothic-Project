...
### フォントファイル（OFL 1.1ライセンス）
**対象範囲：** `fonts/`および`sources/`ディレクトリ内のすべてのフォントファイル。
...
**免責事項（非法律条項）：**
このフォントはSIL Open Font License 1.1でリリースされています。ユーザーは現地の法律と規制を遵守しながら自由にフォントを使用できます。作者はこのフォントの使用によるシステム損害または法的紛争について責任を負いません。OFLライセンス自体に免責条項が含まれています。詳細は [LICENSE-OFL.txt](LICENSE-OFL.txt) をご覧ください。

### ビルドツール（MITライセンス）
**対象範囲：** `tools/`、`registry/`、`sources/build.py`、`.github/workflows/`ディレクトリ内のソースコード、スクリプト、設定ファイル。

- 商用目的を含む自由な使用、修正、配布が可能です。
