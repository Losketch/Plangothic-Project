...
### 字體文件（SIL Open Font License 1.1）

字體文件以 SIL Open Font License 1.1 協議發布。
...
#### 您不可以：
- 將字體文件本身單獨售賣（字體可作為軟件/設備的一部分進行捆綁銷售，但不能單獨作為商品出售）。
- 將字體檔案中的任何部分以非SIL Open Font License授權協議發表。
- 其他上述未列明的違反SIL Open Font License授權協議的行為。
 
**免責聲明（非協議條款）：**
用戶可在遵守當地法律法規的前提下自由使用。作者不對因使用本字體導致的任何系統損壞或法律糾紛承擔責任。OFL 協議本身已包含免責條款，詳見 [LICENSE-OFL.txt](LICENSE-OFL.txt)。

### 構建與開發代碼（MIT License）
- 倉庫中的源代碼、腳本與配置文件以 MIT License 發布，可自由使用、修改、分發，包括商業用途。

---

**重要提醒：**
- 使用本倉庫中的任何內容，即默認視為同意遵守相應協議，詳細的條款內容請參見[LICENSE-OFL.txt](LICENSE-OFL.txt)和[LICENSE-MIT.txt](LICENSE-MIT.txt)。
- 本字體為可免費商用的字體，並以OFL協議發布，任何人均可免費使用、修改和分發。作者從未授權任何個人或機構**單獨售賣本字體文件**。若您在獲取字體文件時被要求支付費用，相關行為可能存在誤導或詐騙風險，請務必謹慎。需要說明的是，OFL協議允許將字體作為其他付費軟件或硬件的一部分進行捆綁分發。
...
- 因為本項目的字體使用性質側重於工具字體、亦即不負責排版僅負責顯示（類似於[Unifont](https://unifoundry.com/unifont)），使用複雜文種處理準確地排版這些語言文字，或是使用異體字選擇器顯示其它地區字形的功能**並不在**本項目的涵蓋範圍內。若您有類似想法，可自行製作有關於您需求的該項目的衍生字體版本。
...
### 6. 各個發行版本有什麼區別？
- **Super版本**：包含所有格式的字體文件，文件體積較大。
- **Static版本**：包含P1和P2兩個獨立的TTF字體文件以及一個TTC文件。
- **OTF版本**：OpenType格式的字體文件以及一個TTC/OTC文件。
- **Web版本**：WOFF2格式的網頁字體文件。
- **單獨的TTF文件**：是從Static中直接提取的，方便單獨下載使用。

**壓縮格式**：.7z和.zip文件內容相同，僅壓縮率不同。.7z體積更小但可能需要第三方解壓軟件，.zip體積稍大但系統自帶支持。
...
4. ***Planseal Project（遍篆體項目）***：基於《說文解字》小篆體，對中日韓越統一表意文字擴展區進行字形補充的項目。
    - **項目狀態**：由於Unicode 18.0新增小篆區塊，因此其與遍黑體項目、遍蕤體項目、遍玨體項目合併，成為其中的一部分。
