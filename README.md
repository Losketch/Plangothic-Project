...
<h1>遍黑体项目（Plangothic Project）</h1>
...
#### 您不可以：
- 将字体文件本身单独售卖（字体可作为软件/设备的一部分进行捆绑销售，但不能单独作为商品出售）。
...
- 本字体为可免费商用的字体，并以OFL协议发布，任何人均可免费使用、修改和分发。作者从未授权任何个人或机构**单独售卖本字体文件**。若您在获取字体文件时被要求支付费用，相关行为可能存在误导或诈骗风险，请务必谨慎。需要说明的是，OFL协议允许将字体作为其他付费软件或硬件的一部分进行捆绑分发。
...
### 6. 各个发行版本有什么区别？
- **Super版本**：包含所有格式的字体文件，文件体积较大。
- **Static版本**：包含P1和P2两个独立的TTF字体文件以及一个TTC文件。
- **OTF版本**：OpenType格式的字体文件以及一个TTC/OTC文件。
- **Web版本**：WOFF2格式的网页字体文件。
- **单独的TTF文件**：是从Static中直接提取的，方便单独下载使用。

**压缩格式**：.7z和.zip文件内容相同，仅压缩率不同。.7z体积更小但可能需要第三方解压软件，.zip体积稍大但系统自带支持。
...
1. **安装字体**：下载TTF版字体文件（两个单独的ttf或者单个的ttc文件任选其一），右键选择"为所有用户安装"（需要管理员权限）；
2. **配置后备**：双击运行 `registry/Plangothic-Install.reg`（需要管理员权限）
3. **重新登录**：注销并重新登录Windows账户使设置生效
4. **卸载**：删除字体文件即可，或运行 `registry/Plangothic-Uninstall.reg` 移除注册表配置

> **高级用法和详细说明**：请查看 [Experimental_README.md](./registry/Experimental/README.md)
</details>
...
4. ***Planseal Project（遍篆体项目）***：基于《说文解字》小篆体，对中日韩越统一表意文字扩展区进行字形补充的项目。
    - **项目状态**：由于Unicode 18.0新增小篆区块，因此其与遍黑体项目、遍蕤体项目、遍玨体项目合并，成为其中的一部分。
